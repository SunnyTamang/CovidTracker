...
public class StateWiseDataActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        last_updated = findViewById(R.id.last_updated_time);
        ...
        bckBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                StateWiseDataActivity.super.onBackPressed();
            }
        });
        ...
        call.enqueue(new Callback<List<LastUpdated>>() {
            ...
            public void onResponse(Call<List<LastUpdated>> call, Response<List<LastUpdated>> response) {
                if (!response.isSuccessful()) {
                    Toast.makeText(getApplicationContext(), "Seems there is issue with the server. Please try after sometimes", Toast.LENGTH_LONG).show();
            ...
            public void onFailure(Call<List<LastUpdated>> call, Throwable t) {
                Toast.makeText(getApplicationContext(), "There seems to be issue with connectivity. Please check your internet connection and try again!", Toast.LENGTH_LONG).show();
            }
        });
    }
...
//                data_add_all.addAll(listOfConfirm);
                ...
                for (int i=0;i<listOfData.size();i++) {
                    if (listOfData.get(i) != null && listOfData.get(i).equals("Unknown")) {
                        listOfData.remove(i);
                        listOfConfirmed.remove(i);
                        listOfActive.remove(i);
                        listOfDeceased.remove(i);
                        listOfRecovered.remove(i);
                    }
                }
